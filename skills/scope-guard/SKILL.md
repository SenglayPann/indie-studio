...
| Monetization and release | New SDK, store text, screenshots, privacy declarations? |
...
5. **Offer options, with a recommendation:**
   - **Park it** (default when it does not strengthen T1): add to `studio/PARKING_LOT.md` with the cost
     and when to revisit.
...
that adds no new system. Refuse to create `feature/*` branches (see `indie-studio:git-workflow`).
